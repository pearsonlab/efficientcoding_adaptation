...
class Encoder(nn.Module):
    def __init__(self,
        ...
        if shape is not None:
            ...
            self.shape_function = shape_module(kernel_size, DoG_version, self.J if individual_shapes else 1)
...
class OutputTerms(object):
    ...
    def calculate_metrics(self, i) -> "OutputMetrics":
        ...
        linear_power = 4.093 #This is the average power for a firing rate of 1. 
        ...
        if 'i' in target:
            ...
            target = float(target)
        if self.model.encoder.nonlinearity == 'linear':
            h = torch.var(self.r) - linear_power
        else:
...
class Retina(nn.Module):
    def __init__(self,
        ...
        self.encoder = Encoder(kernel_size, neurons, frames, temporal_kernel_size,
                               ...
                               individual_shapes, data_covariance, temporal_filter_type, fix_first_two_centers, DoG_version)
