...
def train(logdir: str = datetime.now().strftime(f"{gettempdir()}/%y%m%d-%H%M%S"),
          ...
          frames: int = 1,  # number of frames, T
          jittering_start: Optional[int] = 20000,  # method that jitters the kernels for faster optimization
          jittering_stop: Optional[int] = 100000,
          jittering_interval: int = 1000,
          jittering_power: float = 0.5,
          centering_weight: float = 0.02,  # method that helps kernels localize (not used in the NeurIPS submission)
          centering_start: int = 100000,
          centering_stop: int = 400000,
          input_noise: float = 0.4,  # sigma_in (standard deviation of the Gaussian input noise)
          output_noise: float = 1.2,  # sigma_out (standard deviation of the Gaussian output noise)
          nonlinearity: str = "linear",  # ["softplus", "relu", "linear", "absolute"]
          ...
          learning_rate: float = 0.0001, #Originally 0.001. 0.0001 is better. 
    ...
    if data == "kyoto":
        ...
        dataset = get_dataset(data, kernel_size, data_frames, circle_masking, group_size, random_flip, neural_type, input_noise, device=device)
    ...
    print(input_noise, output_noise, neurons)
    model_args = dict(
    ...
    if load_checkpoint is not None:
        checkpoint = torch.load('/home/david/Gibhut/saves/' + load_checkpoint + '/' + load_checkpoint + '.cp.')
    ...
    with trange(last_iteration + 1, iterations + 1, ncols=99) as loop:
        for iteration in loop:
            ...
            if iteration % 50000 == 0 or iteration == 1:
